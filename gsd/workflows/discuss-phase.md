...
Parse `$ARGUMENTS` for the phase number and `--auto`, then gather context:

```bash
phase_data=$(anvil-cc init phase-op "$phase")
state=$(anvil-cc state json --raw)
```

Use `$phase_data` and `$state` below.
## Initialization Context (from the setup block above)
...
1. **anvil-researcher** - Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **anvil-planner** - Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/anvil-discuss-phase --text`)
- Per-project: `anvil-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/anvil-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /anvil-progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" - Capture context, then run /anvil-plan-phase {X} ${GSD_WS} to replan
...
```bash
TODO_MATCHES=$(anvil-cc todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH=".claude/anvil/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(anvil-cc resolve-model anvil-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @.claude/agents/anvil-advisor-researcher.md for your role and instructions.
...
```markdown
---
author: <your-role-name>     # the standing agent role running this discussion (e.g., architect-lead, mgr); peer-coms addressable owner
---
...
~~~yaml
disciplines:
  - section: "§N.M or inline"
    name: "<short handle>"
    scope: ["<artifacts / symbols this lens is surfaced for>"]
    guidance: "<operational form of the lens>"
    rationale: "<why surfaced for this phase>"
    override_signals:
      - "<concrete situation where overriding is reasonable>"
~~~

[If no lenses surfaced for this phase: "No disciplines surfaced — phase scope didn't call for any."]
...
**Populating `<disciplines>` (the lens section):**

While synthesizing the conversation, watch for moments where the user expressed a *perspective* on the work — not a concrete decision (those go in `<decisions>`), but a way of looking at the work that should inform downstream agents. Examples that read as lenses, not decisions:

- "I want these operations to be pure functions" (perspective on testability and reasoning) → lens
- "Validation belongs at the boundary, not inside the core" (perspective on layering) → lens
- "Errors should be explicit, not silent" (perspective on failure mode visibility) → lens
- "Crash-safe writes" (perspective on durability) → lens

For each surfaced lens, populate one entry in `<disciplines>` with:
- `guidance` — the operational form of the lens (what to look out for when working in scope).
- `rationale` — why the user surfaced it for this phase (their reasoning, paraphrased).
- `override_signals` — **at least one concrete, plausibly-occurring condition** under which a reasonable engineer would override the lens. If the user mentioned one, use it. If not, propose one based on the domain (e.g., for purity: "performance-critical hot path where copy cost is provably the bottleneck").
- `section` — use `"inline"` for v1. The canonical catalogue at `gsd/disciplines/` will eventually hold reusable entries with `§N.M` references, but it's empty at first and grows from observed repetition.

**The override-signals test (load-bearing — read carefully):** If you cannot describe at least one realistic condition under which a competent engineer working in good faith would reasonably override this guidance, **it is not a lens. It is a constraint.** A constraint belongs in the PLAN's `must_haves.truths`, not in CONTEXT's `<disciplines>`. Surfacing a constraint as a lens (e.g., with `override_signals: None` or `override_signals: ["None — this constraint is non-negotiable"]`) corrupts the entire framing: downstream verifier engagement becomes mechanical pass/fail, the three-way outcome model (honored / refined / diverged-with-reasoning) collapses to 100% `honored`, and the engagement signal becomes useless.

**Empirical evidence this discipline matters.** Across testbeds v4, v5, and v6 of the harness, the verifier produced 50+ discipline-engagement outcomes. *Zero* were `refined` or `diverged-with-reasoning` — all `honored`. Inspection of the CONTEXT.md files surfaced the root cause: lenses were surfaced whose `override_signals` were literally "None" or described conditions that never occur in the actual project. Those entries are constraints in disguise. The verifier was correctly observing "no override condition fired, lens honored." The framing was right; the data going in was wrong.

**Apply the test before writing each entry.** For each candidate lens, ask yourself: "What does the engineer do if condition X arises?" If your answer is "still apply the lens — there is no acceptable override," move the entry to the PLAN's `must_haves` and do not surface it as a discipline. If your answer is "they could reasonably set the lens aside because Y" — that IS the override_signal, and the entry belongs in disciplines.

Concrete examples from prior testbeds (the bad-shape ones are anti-patterns):

- BAD: `crash-safe-writes` with `override_signals: None — non-negotiable`. This is a must-have, not a lens. Move to `must_haves.truths` ("`Save()` must write atomically: temp-file + rename pattern; mid-write crash must not corrupt the store").
- BAD: `pure-functions` with `override_signals: performance-critical hot path` when the project is a personal task manager that will never have a hot path. The override is theoretical, not plausible — the lens functions as a constraint.
- GOOD: `validation-at-boundary` with `override_signals: ["pre-validated trusted source (internal microservice that already validated)", "ratchet pattern where outer layer validates and inner can assume safety"]`. These conditions actually occur in real systems; the engineer could honestly reason past the lens.
- GOOD: `explicit-errors` with `override_signals: ["Close() after a read-only open where the data was already consumed", "best-effort cleanup paths where the error is unactionable"]`. Real cases where silent error-swallowing is the right choice.

If applying this test removes ALL candidate disciplines, that is the correct outcome — emit the placeholder "No disciplines surfaced — phase scope didn't call for any (after applying the override-signals test)." Zero honest lenses is better than three dishonest ones.

**Do not invent lenses to fill the section.** If no perspective worth surfacing came up in the conversation, leave the placeholder text ("No disciplines surfaced — phase scope didn't call for any."). Inventing lenses teaches downstream agents to mistake noise for signal — the system gets *worse*, not better.

Also: set the frontmatter `author:` field to your standing-agent role name (whatever agent ran this discuss-phase conversation).
...
**Also available:**
- `/anvil-plan-phase ${PHASE} --skip-research ${GSD_WS}` - plan without research
- `/anvil-ui-phase ${PHASE} ${GSD_WS}` - generate UI design contract before planning (if phase has frontend work)
...
```bash
anvil-cc commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
anvil-cc commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
2. **Sync chain flag with intent** - if user invoked manually (no `--auto`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]]; then
     anvil-cc config-set workflow._auto_chain_active false 2>/dev/null
   fi
   ```
3. Read both the chain flag and user preference:
   ```bash
   AUTO_CHAIN=$(anvil-cc config-get workflow._auto_chain_active 2>/dev/null || echo "false")
   AUTO_CFG=$(anvil-cc config-get workflow.auto_advance 2>/dev/null || echo "false")
   ```
...
```bash
anvil-cc config-set workflow._auto_chain_active true
```
...
```
Skill(skill="anvil-plan-phase", args="${PHASE} --auto ${GSD_WS}")
```
...
- **PHASE COMPLETE** → Full chain succeeded. Display:
  ...
  Next: /anvil-discuss-phase ${NEXT_PHASE} --auto ${GSD_WS}
  <sub>/new first → fresh context window</sub>
  ```
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ```
  Auto-advance partial: Planning complete, execution did not finish.
  Continue: /anvil-execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /anvil-plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /anvil-plan-phase ${PHASE} --gaps ${GSD_WS}
  ```
