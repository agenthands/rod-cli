...
---
<anvil_role_map>
**Read first — anvil 5-peer model (`docs/05-team-workflow.md`).** Milestone kickoff is
the **architect's**. The only spawnable specialists here:
...
Parse `$ARGUMENTS` for flags (`--auto`, `--skip-research`), then gather context:
...
- Check for MILESTONE-CONTEXT.md (from /anvil-discuss-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/anvil-new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/anvil-plan-phase` behavior. To change the default, use `/anvil-settings`.
...
Spawn 4 parallel anvil-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: .claude/anvil/templates/research-project/{FILE}
</output>
", subagent_type="anvil-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
anvil-cc commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
anvil-cc commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/anvil-discuss-phase [N] ${GSD_WS}` - gather context and clarify approach

<sub>`/new` first → fresh context window</sub>

Also: `/anvil-plan-phase [N] ${GSD_WS}` - skip discussion, plan directly
```
...
- [ ] anvil-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/anvil-discuss-phase [N] ${GSD_WS}`
