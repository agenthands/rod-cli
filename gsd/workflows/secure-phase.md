<purpose>
Verify security for a phase/milestone. In the **anvil 5-peer model** this span is
**owned by the security-engineer teammate**, not run by the lead.
</purpose>

<anvil_delegation_directive>
**Read first — governs this workflow.** Per `docs/05-team-workflow.md`, the security
span is the **security-engineer's**. The lead delegates via the **SECURITY handshake**
(version, audit verdict, pointers); the security-engineer runs its discipline
(`agents/security-engineer.md`):
1. **`anvil-threat-modeler`** — enumerate the attack surface from the real code.
2. **`anvil-security-auditor`** — verify: taint / noninterference + the **cert engine**
   on the taint classes (`extract_property_obligation`; `CutFails` = proven exploit).
   Dual context-blind audit on the milestone's keystone claim.
3. **`anvil-spec-author`** — codify recurring security invariants as durable
   `.smtc/analyzers` checks.
Findings flow UP to the lead (never lateral to the engineer); severity reflects
evidence; refuse over hallucinate. The threat-register check below is one part of the
auditor's job, not the whole span.
</anvil_delegation_directive>
...
Valid anvil subagent types (use exact names — do not fall back to 'general-purpose'):
- anvil-threat-modeler — Enumerates the attack surface + threats from real code
- anvil-security-auditor — Deep taint/noninterference audit + cert engine + mitigation coverage
- anvil-spec-author — Codifies security invariants as re-runnable analyzer specs
</available_agent_types>
...
```bash
INIT=$(anvil-cc init phase-op "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_AUDITOR=$(anvil-cc agent-skills anvil-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(anvil-cc resolve-model anvil-security-auditor --raw)
SECURITY_CFG=$(anvil-cc config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /anvil-settings."
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /anvil-execute-phase {N} first."
...
## 5. Spawn anvil-security-auditor
...
```
Agent(
  prompt="Read .claude/agents/anvil-security-auditor.md for instructions.\n\n" +
  ...
  subagent_type="anvil-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `.claude/anvil/templates/SECURITY.md`
...
▶ Fix mitigations then re-run: /anvil-secure-phase {N}
...
```bash
anvil-cc commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
▶ /anvil-validate-phase {N}    validate test coverage
▶ /anvil-verify-work {N}       run UAT
```
