...
Parse `$ARGUMENTS` for the phase number, then:
...
# config settings can be fetched via anvil-tools config-get if needed
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="anvil-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn anvil-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
anvil-cc phases list --type summaries --raw
...
See `.claude/anvil/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
anvil-cc commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See .claude/anvil/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(anvil-cc config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/anvil/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `.claude/anvil/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `.claude/anvil/templates/summary.md`.
...
Update STATE.md using anvil-tools:

```bash
# Advance plan counter (handles last-plan edge case)
anvil-cc state advance-plan

# Recalculate progress bar from disk state
anvil-cc state update-progress

# Record execution metrics
anvil-cc state record-metric \
...
anvil-cc state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
anvil-cc state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using anvil-tools:
...
```bash
anvil-cc roadmap update-plan-progress "${PHASE}"
```
...
```bash
anvil-cc requirements mark-complete ${REQ_IDS}
```
...
```bash
anvil-cc commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
anvil-cc commit "" --files .planning/codebase/*.md --amend
```
</step>
...
| ------------------------------------------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| summaries < plans                          | **A: More plans**     | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/anvil-execute-phase {phase}` + `/anvil-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done**     | Show completion, suggest `/anvil-plan-phase {Z+1}` + `/anvil-verify-work {Z}` + `/anvil-discuss-phase {Z+1}`                                                  |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/anvil-complete-milestone` + `/anvil-verify-work` + `/anvil-add-phase`                                                                  |
