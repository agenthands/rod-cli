...
transitions:
  - from: discussion
    to: planning
    actor: architect
    required_records: [CONTEXT]
  - from: planning
    to: execution_accepted
    actor: engineer
    required_records: [PLAN]
---
<anvil_role_map>
**Read first — anvil 5-peer model (`docs/05-team-workflow.md`).** Planning is the
**architect's**. On opus you ARE the planner: **plan in-band** for most phases. Spawnable:
- **`anvil-planner`** — effort, *scope-gated*: spawn only when the plan is large/parallel.
- **`anvil-researcher`** — for an external-knowledge gap; **`anvil-design-options`** on a genuine fork.

**Legacy role NOT spawnable — do not `Task()` it:**
- `anvil-plan-checker` → the PLAN's independent review is the **engineer's accept/refuse
  PLAN handshake** (the `planning → execution_accepted` transition is `actor: engineer` —
  a different role reviews the plan, which IS the cross-role independence). Where a step
  says spawn `anvil-plan-checker`, instead **hand the PLAN to the engineer** for accept/refuse.
</anvil_role_map>

## Context setup — run these first

Parse `$ARGUMENTS` for the phase number and flags (`--auto`, `--skip-research`, `--gaps`, `--skip-verify`, `--reviews`, `--text`), then gather context:

```bash
phase_data=$(anvil-cc init plan-phase "$phase")
state=$(anvil-cc state json --raw)
roadmap_phase=$(anvil-cc roadmap get-phase "$phase")
```

Use `$phase_data`, `$state`, and `$roadmap_phase` in the steps below.
## Planning Context (from the setup block above)
...
<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates anvil-researcher, anvil-planner, and anvil-plan-checker agents with a revision loop (max 3 iterations).
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@.claude/anvil/references/ui-brand.md
</required_reading>
...
**If `planning_exists` is false:** Error - run `/anvil-new-project` first.
...
/anvil-review --phase {N}

Then re-run /anvil-plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(anvil-cc roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
anvil-cc commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(anvil-cc config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call - AskUserQuestion
  ...
  ```
  Run this command first, then re-run /anvil-plan-phase {X} ${GSD_WS}:

  /anvil-discuss-phase {X} ${GSD_WS}
  ```
...
### Spawn anvil-researcher

```bash
PHASE_DESC=$(anvil-cc roadmap get-phase "${PHASE}" --pick section)
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /anvil-discuss-phase)
...
Task(
  ...
  subagent_type="anvil-researcher",
...
**If found:**
1. Read template: `.claude/anvil/templates/VALIDATION.md`
...
```bash
UI_PHASE_CFG=$(anvil-cc config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(anvil-cc config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(anvil-cc roadmap get-phase "${PHASE}" 2>/dev/null)
...
1. Generate UI-SPEC first - Run /anvil-ui-phase {N} then re-run /anvil-plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/anvil-ui-phase {N} ${GSD_WS}` then re-run `/anvil-plan-phase {N} ${GSD_WS}`". Exit workflow.
...
1. Re-run: `/anvil-plan-phase {PHASE} --research ${GSD_WS}`
2. Disable Nyquist with the exact command:
   `anvil-cc config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn anvil-planner Agent
...
<downstream_consumer>
Output consumed by /anvil-execute-phase. Plans need:
...
## 10. Spawn anvil-plan-checker Agent
...
Task(
  ...
  subagent_type="anvil-plan-checker",
  model="{checker_model}",
...
```
Skill(skill="anvil-execute-phase", args="${PHASE} --auto --no-transition ${GSD_WS}")
```
...
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /anvil-execute-phase ${PHASE} ${GSD_WS}
  ```
...
- /anvil-plan-phase {X} --research - re-research first
- /anvil-review --phase {X} --all - peer review plans with external AIs
- /anvil-plan-phase {X} --reviews - replan incorporating review feedback
...
5. **Retry:** Restart Claude Code and run `/anvil-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/anvil-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] anvil-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] anvil-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] anvil-plan-checker spawned with CONTEXT.md
