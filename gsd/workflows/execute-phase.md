...
transitions:
  ...
  - from: execution_in_progress
    ...
    actor: engineer
    required_records: [SUMMARY]
---
## Context setup — run these first

Parse `$ARGUMENTS` for the phase number and flags (`--auto`, `--no-transition`, `--gaps-only`, `--interactive`, optional user-text), then gather context:

```bash
init=$(anvil-cc init execute-phase "$phase")
case "$init" in @file:*) init=$(cat "${init#@file:}");; esac   # dereference @file: pointer
agent_skills=$(anvil-cc agent-skills anvil-executor 2>/dev/null)
```

Use `$init` and `$agent_skills` in the steps below.

## Execution Context (from the setup block above)
...
<purpose>
Execute all plans in a phase. In the **anvil 5-peer model**, execution is **owned by
the engineer teammate**, not run by the lead as an anonymous subagent orchestrator.
</purpose>

<anvil_delegation_directive>
**Read this first — it governs the rest of this workflow.** Per the canonical team
workflow (`docs/05-team-workflow.md`), the lead does **not** execute a phase by
spawning anonymous `anvil-executor` waves. Instead:

1. The lead hands the **accepted PLAN to the engineer** via the **PLAN handshake**
   (the engineer reviews → accepts/refuses).
2. The **engineer runs its own execute discipline** (see `agents/engineer.md`,
   "Doing the execute-phase work") — which **includes the independent
   `anvil-code-reviewer` gate before the SUMMARY handoff** (a process step, NOT
   optional, NOT scope-gated), plus `anvil-adversarial-tester` against the hard bar.
   The engineer spawns `anvil-executor` *itself* only when the build is large or
   parallel; small/sequential phases it executes in-band — that scope-gating is the
   engineer's call.
3. The engineer returns the SUMMARY up the spine; the lead routes it onward (qa
   verify). **The lead's job here is to delegate + collect, not to execute.**

The wave-based orchestration machinery below is the **fallback** for runtimes or
configurations with **no engineer teammate** (e.g. a solo-orchestrator run). When the
engineer teammate exists, prefer delegation; the code-review gate must hold either
way (if you fall back to anonymous executors, the lead spawns `anvil-code-reviewer`
on the change before accepting the SUMMARY).
</anvil_delegation_directive>

<core_principle>
(Fallback orchestration — only when no engineer teammate is available.) Orchestrator coordinates, not executes. Each subagent loads the full execute-plan context. Orchestrator: discover plans → analyze deps → group waves → spawn agents → handle checkpoints → collect results.
</core_principle>

<runtime_compatibility>
**Subagent spawning is runtime-specific:**
- **Claude Code:** Uses `Task(subagent_type="anvil-executor", ...)` - blocks until complete, returns result
...
- anvil-executor - Executes plan tasks, commits, creates SUMMARY.md
- anvil-verifier - Verifies phase completion, checks quality gates
- anvil-planner - Creates detailed plans from phase scope
- anvil-researcher - Researches technical approaches for a phase
- anvil-plan-checker - Reviews plan quality before execution
- anvil-debugger - Diagnoses and fixes issues
- anvil-codebase-mapper - Maps project structure and dependencies
- anvil-integration-checker - Checks cross-phase integration
- anvil-nyquist-auditor - Validates verification coverage
- anvil-ui-researcher - Researches UI/UX approaches
- anvil-ui-checker - Reviews UI implementation quality
- anvil-ui-auditor - Audits UI against design requirements
</available_agent_types>
...
Check if the current runtime is Copilot by testing for the `@anvil-executor` agent pattern
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `.claude/anvil/workflows/execute-plan.md` **inline**
...
```bash
anvil-cc state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(anvil-cc phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For anvil-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @.claude/anvil/workflows/execute-plan.md
       @.claude/anvil/templates/summary.md
       @.claude/anvil/references/checkpoints.md
       @.claude/anvil/references/tdd.md
       </execution_context>
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    anvil-cc verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
POST_PLAN_INDEX=$(anvil-cc phase-plan-index "${PHASE_NUMBER}")
```
...
/anvil-execute-phase {phase} ${GSD_WS}                # Continue remaining waves
/anvil-execute-phase {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(anvil-cc find-phase "${PARENT_PHASE}" --raw)
...
```bash
anvil-cc commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
VERIFIER_SKILLS=$(anvil-cc agent-skills anvil-verifier 2>/dev/null)
```
...
| `gaps_found`   | Present gap summary, offer `/anvil-plan-phase {phase} --gaps ${GSD_WS}` |
...
Commit the file:
```bash
anvil-cc commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` - they will appear in `/anvil-progress` and `/anvil-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/anvil-verify-work` on it.
...
`/anvil-plan-phase {X} --gaps ${GSD_WS}`

<sub>`/new` first → fresh context window</sub>

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` - full report
Also: `/anvil-verify-work {X} ${GSD_WS}` - manual testing first
```

Gap closure cycle: `/anvil-plan-phase {X} --gaps ${GSD_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/anvil-execute-phase {X} --gaps-only ${GSD_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(anvil-cc phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/anvil-progress` and `/anvil-audit-uat`.
```

```bash
anvil-cc commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
anvil-cc commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/anvil-plan-phase {X} --gaps`). No additional routing needed - skip auto-advance.
...
Read and follow `.claude/anvil/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/anvil-transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/anvil-progress ${GSD_WS} - see updated roadmap
/anvil-discuss-phase {next} ${GSD_WS} - discuss next phase before planning
/anvil-plan-phase {next} ${GSD_WS} - plan next phase
/anvil-execute-phase {next} ${GSD_WS} - execute next phase
```
...
<resumption>
Re-run `/anvil-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
