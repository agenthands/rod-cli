...
```bash
reconcile_result=$(anvil-cc state reconcile --raw 2>/dev/null)
roadmap=$(anvil-cc roadmap analyze --raw)
state=$(anvil-cc state json --raw)
workstreams=$(anvil-cc workstream list --raw 2>/dev/null)   # workstream verb: B1-followup port
```

Use `$reconcile_result`, `$roadmap`, `$state`, `$workstreams` below.

## Context (from the setup block above)
...
There is no `/anvil-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/anvil-transition`.

**Valid user commands for phase progression:**
- `/anvil-discuss-phase {N}` - discuss a phase before planning
- `/anvil-plan-phase {N}` - plan a phase
- `/anvil-execute-phase {N}` - execute a phase
- `/anvil-progress` - see roadmap progress
...
These will carry forward as debt. Review: `/anvil-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to anvil-tools:**

```bash
TRANSITION=$(anvil-cc phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `anvil-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(anvil-cc progress bar --raw)
```
...
**Use the transition result from `anvil-tools phase complete`:**
...
```bash
ROADMAP=$(anvil-cc roadmap analyze)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(anvil-cc workstream list --raw)
fi
```
...
Exit skill and invoke SlashCommand("/anvil-plan-phase [X+1] --auto ${GSD_WS}")
...
Exit skill and invoke SlashCommand("/anvil-discuss-phase [X+1] --auto ${GSD_WS}")
...
`/anvil-discuss-phase [X+1] ${GSD_WS}` - gather context and clarify approach
...
**Also available:**
- `/anvil-plan-phase [X+1] ${GSD_WS}` - skip discussion, plan directly
- `/anvil-research-phase [X+1] ${GSD_WS}` - investigate unknowns
...
**Also available:**
- `/anvil-discuss-phase [X+1] ${GSD_WS}` - revisit context
- `/anvil-research-phase [X+1] ${GSD_WS}` - investigate unknowns
...
```bash
anvil-cc config-set workflow._auto_chain_active false
```
...
`/anvil-workstreams complete {current_ws_name} ${GSD_WS}`

See overall milestone progress:

`/anvil-workstreams progress ${GSD_WS}`
...
Do NOT suggest `/anvil-complete-milestone` or `/anvil-new-milestone`.
...
Exit skill and invoke SlashCommand("/anvil-complete-milestone {version} ${GSD_WS}")
...
`/anvil-complete-milestone {version} ${GSD_WS}`
