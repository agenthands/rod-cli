...
agent_skills_debugger=$(anvil-cc agent-skills anvil-debugger 2>/dev/null)
```

Use `$state` and `$agent_skills_debugger` below.
...
```bash
anvil-cc commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
